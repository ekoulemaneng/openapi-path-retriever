    ...
    describe('schemaRetriever', () => {
        describe('throws an error when', () => {
            it('specification argument is not provided', async () => {
                ...
                // Exercise and verify
                await assert.rejects(async () => await schemaRetriever(), expected)
            })
            it('specification argument is neither a string nor an object', async () => {
                ...
                // Exercise and verify
                await assert.rejects(async () => await schemaRetriever(schema), expected)
            })
            it('dirname argument is not a string', async () => {
                ...
                // Exercise and verify
                await assert.rejects(async () => await schemaRetriever(schema, dirname), expected)
            })
            it('specification argument file is not a json, yaml or yml file', async () => {
                ...
                // Exercise and verify
                await assert.rejects(async () => await schemaRetriever(schema, __dirname), expected)
            })
        })
        describe('returns', () => {
            for(let i = 1; i <= 7; i++) {
                describe('an object', () => {
                    context(`for specification ${i}`, () => {
                        ...
                        it('if the specification is an object', async () => {
                            ...
                            // Exercise
                            const actual = typeof await schemaRetriever(schema)
                            // Verify
                        ...
                        it('if the specification is a json file', async () => {
                            ...
                            const expected = 'object'
                            // Exercise
                            const actual = typeof await schemaRetriever(schema, dirname)
                            // Verify
                            assert.strictEqual(actual, expected)
                        })
                ...
                for (let i = 1; i <= 7; i++) {
                    context(`for specification #${i}`, () => {
                        ...
                        it('as json file', async () => {
                            // Setup
                            const schema = await schemaRetriever(`./specs/json/openapi${i}.json`, dirname)
                            const expected = null
                            // Exercise
                            const actual = pathRetriever(schema, url)
                            // Verify
                        ...
                        it('as yaml file', async () => {
                            // Setup
                            const schema = await schemaRetriever(`./specs/yaml/openapi${i}.yaml`, dirname)
                            const expected = null
                            // Exercise
                            const actual = pathRetriever(schema, url)
                            // Verify
